...
static void sync_polygon_mode(const GxmContextState &state) {
    R_PROFILE(__func__);
...
static void sync_point_line_width(const GxmContextState &state) {
    R_PROFILE(__func__);
...
static void sync_depth_bias(const GxmContextState &state) {
    R_PROFILE(__func__);
...
bool sync_state(Context &context, const GxmContextState &state, const MemState &mem, bool enable_texture_cache) {
    ...
    sync_stencil(state);
    sync_polygon_mode(state);
    sync_point_line_width(state);
    sync_depth_bias(state);
